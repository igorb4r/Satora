  ...
  if(!message.member.hasPermission("BAN_MEMBERS")) return message.channel.send("VOCÊ NÃO É ADM!")
    try {
      let output = '';
        ...
        message.channel.send(`**Você quer receber a lista de bans? Reaja com ✅ para confirmar o envio | Caso Nao queira reaja com** ❌ .`)
              .then(async (msg) => {
          await msg.react("✅")
          await msg.react("❌")
                ...
                switch (r.emoji.name) {
                  case '✅':
                    ...
                    message.guild.fetchBans().then(async (bans) => {
                     message.channel.send('Enviei a lista de BANS no seu privado! \n(`Caso não receba nenhuma mensagem no privado significa que não tem NIGUEM BANIDO!`)');
