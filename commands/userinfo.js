...
module.exports = {
  ...
  run: async (client, message, args) => {
    ...
    if (!args[0]) {
      ...
      user = message.mentions.members.first() || await message.guild.members.fetch(args[0]).catch(err => { return message.channel.send(":x: Incapaz de encontrar esta pessoa") })
    }

    if (!user) {
      return message.channel.send(":x: Incapaz de encontrar esta pessoa!")
    }
    ...
    if (user.user.presence.activities.length) {
      ...
      if (user.nickname !== null) embed.addField("Apelido", user.nickname)
      embed.addField("Ingressou em", moment(user.user.joinedAt).format("mm/dd/yyyy"))
        .addField("Conta Criada em", moment(user.user.createdAt).format("mm/dd/yyyy"))
        .addField("Informação Comum", `ID: \`${user.user.id}\`\nDiscriminador: ${user.user.discriminator}\nBot: ${user.user.bot}\nUsuário deletado: ${user.deleted}`)
        .addField("Distintivos", newbadges.join(",").toLowerCase() || "None")
